...
- *`index.json` budget* — hard cap **4,000 tokens**, target 2,000, enforced by an admission test that fails the build when exceeded. The index carries only id, scope, the one-line applicability question, and file path — roughly 30–40 tokens per entry, so 25 seed patterns fit with room. Growth past the cap forces shortening lines or splitting the index by scope; both beat silent bloat. Distinct from the decision record's ~600-token re-injection budget, because the index is loaded once at selection rather than carried through the session.
- *Slice-3 drift task* — **FinanceOps at `974ee76c`: add one owner-site surface through the contribution path.** Chosen because the trap is known to work on real agents. The enforced path is `OWNER_ARTIFACT.json` plus intake verification; the ceremonial path is `integration.json`. Six owners already took the wrong one. The drift metric is therefore concrete: given the decision record, does the agent write to the load-bearing path or the inert one? Intake verification supplies independent evidence, satisfying the check-independence rule.
