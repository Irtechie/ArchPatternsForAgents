---
slice_id: "02"
title: "Repository-owned contract and consumer adapter"
...
hitl: false
expected_files: ["docs/architecture-patterns/repository-contract.md", "docs/architecture-patterns/proof/02-review.md"]
proof_check: {kind: artifact, path: "docs/architecture-patterns/proof/02-review.md"}
---

# Repository-owned contract and consumer adapter
...
**Invariant:** The provider owns public semantics; the consumer owns its adapter. Communication crosses a published contract, never another repo's internals.

**Concrete flow:** A TypeScript app calls a Python domain API through its own adapter; provider validation remains in the Python repo.

**Bolted-on warning:** Sibling source imports, shared database writes, or consumer retries that reinterpret a provider rejection as permission.

## Contract to specify
Contract version, request/result schemas, typed failures, timeout, retry ownership, compatibility window, contract fixtures.

## Failure cases and required proof
Cases: Old consumer/new provider; removed field; timeout after possible commit; provider unavailable.
Required observable result: Supported older requests remain valid; incompatible requests fail explicitly; an unknown outcome is reconciled rather than silently replayed.
...
6. Complete proof/02-review.md with a field-completeness matrix, invariant-to-scenario mapping, dependency consistency, and language neutrality review. Mark this plan done only when all pass.

## Scope and tradeoff
A contract can use HTTP, messages, CLI, or a library public API as justified. Do not require a shared SDK, protocol, broker, or deploy cadence.
