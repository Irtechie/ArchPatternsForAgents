---
slice_id: "06"
title: "Durable decisions and derived memory"
...
hitl: false
expected_files: ["docs/architecture-patterns/decision-memory.md", "docs/architecture-patterns/proof/06-review.md"]
proof_check: {kind: artifact, path: "docs/architecture-patterns/proof/06-review.md"}
---

# Durable decisions and derived memory
...
**Invariant:** Accepted decisions retain rationale, provenance, and supersession in their owning domain; summaries and assistant recall remain projections.

**Concrete flow:** An application strategy is revised with a reason; a new conversation retrieves the current strategy and its predecessor.

**Bolted-on warning:** Chat transcript used as a database, summary overwrites the accepted decision, or semantic similarity resurrects superseded advice.

## Contract to specify
Decision ID, scope, statement, rationale, evidence, status, revision/supersedes, author, acceptance authority, retention policy.

## Failure cases and required proof
Cases: Contradiction; stale summary; duplicate observation; correction; deletion/retention request; projection loss.
Required observable result: Fresh-context recall selects the accepted current revision; rebuild restores it; forgotten data does not reappear from derived stores.
...
6. Complete proof/06-review.md with a field-completeness matrix, invariant-to-scenario mapping, dependency consistency, and language neutrality review. Mark this plan done only when all pass.

## Scope and tradeoff
Keep operational decisions separate from preferences and learning notes. Retention may require removal; append-only is not an absolute rule.
