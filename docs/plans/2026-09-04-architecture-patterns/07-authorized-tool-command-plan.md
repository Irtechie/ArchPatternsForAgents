---
slice_id: "07"
title: "Validated tool command and action receipt"
...
hitl: false
expected_files: ["docs/architecture-patterns/authorized-tool-command.md", "docs/architecture-patterns/proof/07-review.md"]
proof_check: {kind: artifact, path: "docs/architecture-patterns/proof/07-review.md"}
---

# Validated tool command and action receipt
...
**Invariant:** The model proposes; deterministic code validates schema, actor, scope, and current authority before the owning service performs a side effect.

**Concrete flow:** An assistant proposes a scheduling change; the owner validates it under existing authorization and returns a durable result.

**Bolted-on warning:** Model output executes directly, approval is a vague global flag, or a caller reports success before the owner confirms it.

## Contract to specify
Operation/argument identity, actor, authorization policy, expected revision, idempotency key, result/unknown outcome, receipt.

## Failure cases and required proof
Cases: Prompt injection; changed arguments after approval where approval is required; timeout after commit; duplicate request; expired authority.
Required observable result: An unauthorized or changed command is rejected; repeated identity and payload do not duplicate the action; mismatched payload is rejected; ambiguous completion reconciles.
...
6. Complete proof/07-review.md with a field-completeness matrix, invariant-to-scenario mapping, dependency consistency, and language neutrality review. Mark this plan done only when all pass.

## Scope and tradeoff
Existing user authorization remains valid within scope. Do not add mandatory confirmations to every action or promise universal exactly-once execution.
