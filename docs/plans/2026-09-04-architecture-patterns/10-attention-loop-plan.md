---
slice_id: "10"
title: "Observation to actionable attention"
...
hitl: false
expected_files: ["docs/architecture-patterns/attention-loop.md", "docs/architecture-patterns/proof/10-review.md"]
proof_check: {kind: artifact, path: "docs/architecture-patterns/proof/10-review.md"}
---

# Observation to actionable attention
...
**Invariant:** Background observers compare authoritative state, deduplicate meaningful changes, and route actions through the same authorized command path.

**Concrete flow:** A monitor detects a changed application deadline and records one attention item with evidence and a resolution state.

**Bolted-on warning:** Every poll sends a notification, a scheduler invents domain rules, or background agents acquire broader powers than interactive callers.

## Contract to specify
Observation revision, comparison rule owner, dedup key, attention status, delivery receipt, retry policy, authorized action link.

## Failure cases and required proof
Cases: Unchanged source; noisy oscillation; duplicate tick; offline notifier; stale observation; resolved condition.
Required observable result: Unchanged state creates no new attention; duplicate ticks coalesce; notification failure does not erase the attention item or trigger an unauthorized action.
...
6. Complete proof/10-review.md with a field-completeness matrix, invariant-to-scenario mapping, dependency consistency, and language neutrality review. Mark this plan done only when all pass.

## Scope and tradeoff
Polling and events are interchangeable bindings. This pattern is optional for apps without background work.
