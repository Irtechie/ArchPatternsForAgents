---
slice_id: "12"
title: "Separate behavioral proof from model quality"
...
hitl: false
expected_files: ["docs/architecture-patterns/evaluation-boundary.md", "docs/architecture-patterns/proof/12-review.md"]
proof_check: {kind: artifact, path: "docs/architecture-patterns/proof/12-review.md"}
---

# Separate behavioral proof from model quality
...
**Invariant:** Deterministic invariants, integration readiness, and model-quality evaluations have separate outcomes and independently owned expectations.

**Concrete flow:** Tool authorization tests remain exact while extraction quality is evaluated on a versioned corpus with explicit thresholds.

**Bolted-on warning:** An LLM grades its own implementation against rewritten expectations, a mock proves transport readiness, or output quality hides unsafe side effects.

## Contract to specify
Fixture origin/revision, invariant IDs, consumer probes, quality metrics/thresholds, model/config identity, evidence expiry.

## Failure cases and required proof
Cases: Known invalid action; transport failure; prompt/model change; quality regression; held-out example; missing live environment.
Required observable result: A deliberate invariant violation fails deterministic proof even with a fluent answer; a quality regression cannot be hidden by transport success; unrun live tests remain unknown.
...
6. Complete proof/12-review.md with a field-completeness matrix, invariant-to-scenario mapping, dependency consistency, and language neutrality review. Mark this plan done only when all pass.

## Scope and tradeoff
Use the application's existing test runner. Do not add an evaluation platform or require an LLM judge for exact properties.
